...
Calculates the market penetration based on the number of customers within an area compared to a demographic variable such as total population. An optional report can be created detailing the market penetration.
        ...
        report_format = params.get("report_format")
            ...
            output_name = f"{in_features.replace(' ', '_')}_Calculate_Market_Penetration"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            ...
            arcpy.CalculateMarketPenetration(in_features, out_feature_class, id_field, market_penetration_base_field, in_customer_features, area_description_field, weight_field, create_report, store_id, link_field, report_title, report_folder, report_format)
...
Creates multilevel, scale-dependent choropleth layers from a variable describing a business, demographic, consumer, or landscape characteristic.
        ...
        renderer_type = params.get("renderer_type")
            ...
            # Generate output name and path
            output_name = f"{classification_variable.replace(' ', '_')}_Color_Coded_Layer"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Color Coded Layer
            arcpy.ColorCodedLayer(classification_variable, out_layer_name, classification_method, number_of_classes, area_of_interest, out_dataset_path, out_dataset_name, boundary_mode, secondary_variable, grid_size, renderer_type)
...
Enriches data by adding demographic and landscape facts about the people and places that surround or are inside data locations. The output is a duplicate of the input with additional attribute fields. This tool requires an ArcGIS Online organizational account or a locally stored or installed dataset. This tool uses detailed aggregation and apportionment settings to summarize data.  The Apportion Polygon tool is similar to this tool. However, Apportion Polygon uses user-specified apportionment. Enrich Layer uses U.S. Census Block points or global settlement points for apportionment. For more information, see Data apportionment.
        ...
        unit = params.get("unit")
            ...
            # Generate output name and path
            output_name = f"{in_features.replace(' ', '_')}_Enrich_Layer"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Enrich Layer
            arcpy.EnrichLayer(in_features, out_feature_class, variables, buffer_type, distance, unit)
...
Identifies locations closest to the input features based on a selected distance type. The number of points in the output is defined by limiting the count or percentage of location points to return or by limiting the distance from the input points.
        ...
        location_name = params.get("location_name")
            ...
            # Generate output name and path
            output_name = f"{in_features.replace(' ', '_')}_Find_Nearby_Locations"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Find Nearby Locations
            arcpy.FindNearbyLocations(in_features, id_field, in_location_points, out_feature_class, distance_type, units, distance_limit, number_limit, percent_limit, create_report, report_title, report_folder, report_format, report_fields, travel_direction, time_of_day, time_zone, search_tolerance, location_name)
...
Generates a series of lines from each customer to an associated store location. These lines are often called spider diagrams. The tool can also generate an optional Wind Rose report from the output. Note:This tool is similar to the Generate Origin-Destination Links tool in the Analysis toolbox Proximity toolset.
        ...
        report_format = params.get("report_format")
            ...
            # Generate output name and path
            output_name = f"{in_stores_layer.replace(' ', '_')}_Generate_Desire_Lines"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Generate Desire Lines
            arcpy.GenerateDesireLines(in_stores_layer, in_customers_layer, out_feature_class, store_id_field, link_field, distance_type, units, cutoff, travel_direction, time_of_day, time_zone, create_report, report_title, report_folder, report_format)
...
Creates features with vector-based square grid cells, hexagons, or H3 hexagons for a given area.
        ...
        out_centroids = params.get("out_centroids")
            ...
            # Generate output name and path
            output_name = f"{area_of_interest.replace(' ', '_')}_Generate_Grids_and_Hexagons"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Generate Grids and Hexagons
            arcpy.GenerateGridsandHexagons(area_of_interest, out_feature_class, cell_type, enrich_type, cell_size, h3_resolution, variables, distance_type, distance, units, out_enriched_buffers, travel_direction, time_of_day, time_zone, search_tolerance, polygon_detail, out_centroids)
...
Generates a point feature layer from a business point location search.
        ...
        filtersfilter_name_filter_value_include = params.get("filtersfilter_name_filter_value_include")
        ...
        style = params.get("style")
            ...
            # Generate output name and path
            output_name = f"{out_feature_class.replace(' ', '_')}_Generate_Points_from_Business_Listings"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Generate Points from Business Listings
            arcpy.GeneratePointsfromBusinessListings(out_feature_class, in_search_features, search_terms, exact_match, match_name_only, filtersfilter_name_filter_value_include, max_count, business_dataset, find_related_poi, style)
...
Creates a probability surface to predict the sales potential of an area based on distance and an attractiveness factor.
        ...
        if out_feature_class is None:
            return {"success": False, "error": "out_feature_class parameter is required"}
        attractiveness_variablesexisting_facilities_value_candidates_location_value_exponent = params.get("attractiveness_variablesexisting_facilities_value_candidates_location_value_exponent")
        if attractiveness_variablesexisting_facilities_value_candidates_location_value_exponent is None:
            return {"success": False, "error": "attractiveness_variablesexisting_facilities_value_candidates_location_value_exponent parameter is required"}
        ...
        time_zone = params.get("time_zone")
            ...
            # Generate output name and path
            output_name = f"{in_facility_features.replace(' ', '_')}_Huff_Model"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Huff Model
            arcpy.HuffModel(in_facility_features, facility_id_field, in_candidate_features, candidate_id_field, in_sales_potential_features, sales_potential_id_field, sales_potential_field, out_feature_class, attractiveness_variablesexisting_facilities_value_candidates_location_value_exponent, distance_exponent, distance_type, distance_units, out_distance_matrix, travel_direction, time_of_day, time_zone)
...
Calculates exponent values for use in the Huff Model tool.
        ...
        time_zone = params.get("time_zone")
            ...
            # Generate output name and path
            output_name = f"{in_facility_features.replace(' ', '_')}_Huff_Model_Calibration"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Huff Model Calibration
            arcpy.HuffModelCalibration(in_facility_features, facility_id_field, in_customer_features, link_field, in_sales_potential_features, sales_potential_id_field, out_calibration, attractiveness_variables, customer_weight_field, distance_type, distance_units, travel_direction, time_of_day, time_zone)
...
Imports an online report so that it can run locally using a locally installed ArcGIS Business Analyst dataset. This tool is the underlying engine that runs the Import Template interactive workflow from the Catalog pane. This tool is ideal for performing more advanced automated tasks, such as importing templates through Python scripting or programmatically modifying data paths. The Import Template workflow will run the tool in the background and add its information to the Geoprocessing history.
        ...
        config = params.get("config")
            ...
            # Generate output name and path
            output_name = f"{online_report_template_id.replace(' ', '_')}_Import_Business_Analyst_Report_Template"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Import Business Analyst Report Template
            arcpy.ImportBusinessAnalystReportTemplate(online_report_template_id, output_folder, dataset_id, config)
...
Creates infographic and summary reports for a boundary layer using standard or custom templates.
        ...
        report_style = params.get("report_style")
            ...
            # Generate output name and path
            output_name = f"{in_features.replace(' ', '_')}_Summary_Reports"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Summary Reports
            arcpy.SummaryReports(in_features, report_templates, reports_folder, summarization_options, single_report, formats, store_id_field, store_name_field, store_address_field, store_latitude_field, store_longitude_field, ring_id_field, area_description_field, title, subtitle, report_per_feature, add_infographic_header, add_infographic_footer, add_infographic_data_source, report_style)
...
Creates an index for a Statistical Data Collection (SDCX). The index will improve performance when using the custom data in analysis tools such as Enrich Layer.
        ...
        if sdcx_file is None:
            ...
            # Generate output name and path
            output_name = f"{sdcx_file.replace(' ', '_')}_Generate_SDCX_Index"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Generate SDCX Index
            arcpy.GenerateSDCXIndex(sdcx_file)
...
Adds criteria based on the numerical fields existing in the input layer.
        ...
        if fields is None:
            ...
            # Generate output name and path
            output_name = f"{in_analysis_layer.replace(' ', '_')}_Add_Field_Based_Suitability_Criteria"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Add Field Based Suitability Criteria
            arcpy.AddFieldBasedSuitabilityCriteria(in_analysis_layer, fields)
...
Adds criteria based on spatial relationships between the input layer and a specified point layer.
        ...
        cutoff_distance = params.get("cutoff_distance")
            ...
            # Generate output name and path
            output_name = f"{in_analysis_layer.replace(' ', '_')}_Add_Point_Layer_Based_Suitability_Criteria"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Add Point Layer Based Suitability Criteria
            arcpy.AddPointLayerBasedSuitabilityCriteria(in_analysis_layer, site_layer_id_field, in_point_features, criteria_type, distance_type, units, in_site_centers_features, site_centers_id_field, weight_field, statistics_type, cutoff_distance)
...
Adds criteria based on the values calculated for the input layer using the Enrich Layer tool.
        ...
        if variables is None:
            ...
            # Generate output name and path
            output_name = f"{in_analysis_layer.replace(' ', '_')}_Add_Variable_Based_Suitability_Criteria"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Add Variable Based Suitability Criteria
            arcpy.AddVariableBasedSuitabilityCriteria(in_analysis_layer, variables)
...
Calculates or recalculates a suitability score.
        ...
        if in_analysis_layer is None:
            ...
            # Generate output name and path
            output_name = f"{in_analysis_layer.replace(' ', '_')}_Calculate_Suitability_Score"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Calculate Suitability Score
            arcpy.CalculateSuitabilityScore(in_analysis_layer)
...
Creates a Suitability Analysis group layer for an input site's polygonal layer.
        ...
        out_dataset_name = params.get("out_dataset_name")
            ...
            # Generate output name and path
            output_name = f"{in_features.replace(' ', '_')}_Make_Suitability_Analysis_Layer"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Make Suitability Analysis Layer
            arcpy.MakeSuitabilityAnalysisLayer(in_features, layer_name, out_dataset_path, out_dataset_name)
...
Removes criteria from a suitability analysis layer.
        ...
        if drop_criteria is None:
            ...
            # Generate output name and path
            output_name = f"{in_analysis_layer.replace(' ', '_')}_Remove_Suitability_Criteria"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Remove Suitability Criteria
            arcpy.RemoveSuitabilityCriteria(in_analysis_layer, drop_criteria)
...
Defines parameters for criteria.
        ...
        if in_analysis_layer is None:
            return {"success": False, "error": "in_analysis_layer parameter is required"}
        criteria_propertiescriterion_title_weight_influence_ideal_value_minimum_value_maximum_value_enabled = params.get("criteria_propertiescriterion_title_weight_influence_ideal_value_minimum_value_maximum_value_enabled")
        if criteria_propertiescriterion_title_weight_influence_ideal_value_minimum_value_maximum_value_enabled is None:
            return {"success": False, "error": "criteria_propertiescriterion_title_weight_influence_ideal_value_minimum_value_maximum_value_enabled parameter is required"}
        ...
        final_score_method = params.get("final_score_method")
            ...
            # Generate output name and path
            output_name = f"{in_analysis_layer.replace(' ', '_')}_Set_Criteria_Properties"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Set Criteria Properties
            arcpy.SetCriteriaProperties(in_analysis_layer, criteria_propertiescriterion_title_weight_influence_ideal_value_minimum_value_maximum_value_enabled, criteria_score_preset, preprocessing, criteria_score_method, final_score_method)
...
Adds a target site to a Suitability Analysis group layer. A target site is typically a trade area around a high-performing location, such as a store or clinic. Criteria values in the target site are used to score candidate sites that are closest in value to the target site.
        ...
        target_site_feature_id = params.get("target_site_feature_id")
            ...
            # Generate output name and path
            output_name = f"{in_analysis_layer.replace(' ', '_')}_Set_Target_Site"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Set Target Site
            arcpy.SetTargetSite(in_analysis_layer, target_site_layer, target_site_feature_id)
...
Generates a layer that displays the gap between total customers and expected customers. Learn more about Analyze Market Area Gap outputs
        ...
        report_format = params.get("report_format")
            ...
            # Generate output name and path
            output_name = f"{customer_layer.replace(' ', '_')}_Analyze_Market_Area_Gap"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Analyze Market Area Gap
            arcpy.AnalyzeMarketAreaGap(customer_layer, target_profile, base_profile, geography_level, out_feature_class, target_group, core_target, developmental_target, boundary_layer, create_report, report_title, report_folder, report_format)
...
Generates a layer that displays expected customers by a selected geography level. Learn more about Analyze Market Potential outputs
        ...
        report_format = params.get("report_format")
            ...
            # Generate output name and path
            output_name = f"{target_profile.replace(' ', '_')}_Analyze_Market_Potential"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Analyze Market Potential
            arcpy.AnalyzeMarketPotential(target_profile, base_profile, geography_level, out_feature_class, boundary_layer, create_report, report_title, report_folder, report_format)
...
Creates a new target group. A target group is a container for targets that you create, name, and populate with segments from a locally installed Business Analyst dataset.
        ...
        if input_type is None:
            ...
            # Generate output name and path
            output_name = f"{target_group.replace(' ', '_')}_Create_Target_Group"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Create Target Group
            arcpy.CreateTargetGroup(target_group, input_type)
...
Exports a segmentation profile to a table.
        ...
        if out_table_name is None:
            ...
            # Generate output name and path
            output_name = f"{in_profile.replace(' ', '_')}_Export_Segmentation_Profile"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Export Segmentation Profile
            arcpy.ExportSegmentationProfile(in_profile, out_table_name)
...
Displays characteristics from the consumer survey data for your target profile to determine customer lifestyle habits and preferences.
        ...
        report_format = params.get("report_format")
            ...
            # Generate output name and path
            output_name = f"{target_profile.replace(' ', '_')}_Generate_Survey_Report_For_Profile"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Generate Survey Report For Profile
            arcpy.GenerateSurveyReportForProfile(target_profile, base_profile, survey_category, report_folder, sort_column, sort_order, report_title, report_format)
...
Displays the top characteristics, in each of the selected survey categories, of your Core and Developmental target groups, as well as your overall customer profile.
        ...
        report_format = params.get("report_format")
            ...
            # Generate output name and path
            output_name = f"{target_profile.replace(' ', '_')}_Generate_Survey_Report_For_Targets"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Generate Survey Report For Targets
            arcpy.GenerateSurveyReportForTargets(target_profile, target_group, core_target, developmental_target, report_folder, report_type, survey_categories, report_title, report_format)
...
Generates a layer that identifies geographies that contain selected segments and categorized groups based on targets.
        ...
        boundary_layer = params.get("boundary_layer")
            ...
            # Generate output name and path
            output_name = f"{geography_level.replace(' ', '_')}_Generate_Target_Group_Layer"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Generate Target Group Layer
            arcpy.GenerateTargetGroupLayer(geography_level, segmentation_base, out_feature_class, target_group, boundary_layer)
...
Creates a layer that identifies geographies that contain selected segments and geographies that do not contain selected segments.
        ...
        boundary_layer = params.get("boundary_layer")
            ...
            # Generate output name and path
            output_name = f"{geography_level.replace(' ', '_')}_Generate_Target_Layer"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Generate Target Layer
            arcpy.GenerateTargetLayer(geography_level, segmentation_base, out_feature_class, input_type, target_group, target, segments, boundary_layer)
...
Generates a layer based on the percent of penetration of selected segments, providing a detailed view of the concentrations of the target segments.
        ...
        boundary_layer = params.get("boundary_layer")
            ...
            # Generate output name and path
            output_name = f"{geography_level.replace(' ', '_')}_Generate_Target_Penetration_Layer"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Generate Target Penetration Layer
            arcpy.GenerateTargetPenetrationLayer(geography_level, segmentation_base, out_feature_class, input_type, target_group, target, segments, boundary_layer)
...
Generates a segmentation profile from a table.
        ...
        total_volume_field = params.get("total_volume_field")
            ...
            # Generate output name and path
            output_name = f"{in_table.replace(' ', '_')}_Import_Segmentation_Profile"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Import Segmentation Profile
            arcpy.ImportSegmentationProfile(in_table, segmentation_base, out_profile, segment_id_field, count_field, total_volume_field)
...
Imports segmentation profiles consisting of survey variable data.
        ...
        if out_folder is None:
            ...
            # Generate output name and path
            output_name = f"{profiles.replace(' ', '_')}_Import_Survey_Profiles"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Import Survey Profiles
            arcpy.ImportSurveyProfiles(profiles, out_folder)
...
Assigns customers to the closest store based on a selected distance type.
        ...
        search_tolerance = params.get("search_tolerance")
            ...
            # Generate output name and path
            output_name = f"{in_features.replace(' ', '_')}_Assign_Customers_By_Distance"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Assign Customers By Distance
            arcpy.AssignCustomersByDistance(in_features, in_store_features, store_id_field, out_feature_class, link_field, distance_type, distance_units, travel_direction, time_of_day, time_zone, search_tolerance)
...
Creates trade areas that approximate the size, shape, and area of existing polygons using available routes from the selected distance type.
        ...
        polygon_detail = params.get("polygon_detail")
            ...
            # Generate output name and path
            output_name = f"{in_features.replace(' ', '_')}_Generate_Approximate_Drive_Times"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Generate Approximate Drive Times
            arcpy.GenerateApproximateDriveTimes(in_features, out_feature_class, distance_type, units, in_stores_layer, store_id_field, link_field, iterations_limit, minimum_step, target_percent_diff, travel_direction, time_of_day, time_zone, search_tolerance, polygon_detail)
...
Creates a feature class of trade areas around point features based on travel time and distance.
        ...
        expression = params.get("expression")
            ...
            # Generate output name and path
            output_name = f"{in_features.replace(' ', '_')}_Generate_Drive_Time_Trade_Area"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Generate Drive Time Trade Area
            arcpy.GenerateDriveTimeTradeArea(in_features, out_feature_class, distance_type, distances, units, id_field, dissolve_option, remove_overlap, travel_direction, time_of_day, time_zone, search_tolerance, polygon_detail, input_method, expression)
...
Generates trade areas from the features of an input standard geography level that has a specified spatial relationship with the input.
        ...
        ratios = params.get("ratios")
            ...
            # Generate output name and path
            output_name = f"{geography_level.replace(' ', '_')}_Generate_Geographies_From_Overlay"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Generate Geographies From Overlay
            arcpy.GenerateGeographiesFromOverlay(geography_level, in_features, id_field, out_feature_class, overlap_type, ratios)
...
Creates trade areas based on predefined named statistical areas. This tool does not consume credits.
        ...
        dissolve_output = params.get("dissolve_output")
            ...
            # Generate output name and path
            output_name = f"{geography_level.replace(' ', '_')}_Generate_Standard_Geography_Trade_Areas"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Generate Standard Geography Trade Areas
            arcpy.GenerateStandardGeographyTradeAreas(geography_level, out_feature_class, input_type, in_ids_table, geography_key_field, ids_list, summarize_duplicates, group_field, dissolve_output)
...
Creates a feature class of network distance trade areas that expand around point features until criteria is reached.
        ...
        expression = params.get("expression")
            ...
            # Generate output name and path
            output_name = f"{in_features.replace(' ', '_')}_Generate_Threshold_Drive_Times"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Generate Threshold Drive Times
            arcpy.GenerateThresholdDriveTimes(in_features, out_feature_class, threshold_variable, threshold_values, distance_type, units, id_field, travel_direction, time_of_day, time_zone, search_tolerance, polygon_detail, iterations_limit, minimum_step, target_percent_diff, input_method, expression)
...
Creates a feature class of ring trade areas that expand around point features until the threshold value is reached.
        ...
        target_percent_diff = params.get("target_percent_diff")
            ...
            # Generate output name and path
            output_name = f"{in_features.replace(' ', '_')}_Generate_Threshold_Rings"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Generate Threshold Rings
            arcpy.GenerateThresholdRings(in_features, out_feature_class, threshold_variable, threshold_values, units, id_field, input_method, expression, minimum_step, target_percent_diff)
...
Creates rings around point locations.
        ...
        expression = params.get("expression")
            ...
            # Generate output name and path
            output_name = f"{in_features.replace(' ', '_')}_Generate_Trade_Area_Rings"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Generate Trade Area Rings
            arcpy.GenerateTradeAreaRings(in_features, out_feature_class, radii, units, id_field, remove_overlap, dissolve_option, input_method, expression)
...
Calculates the amount of overlap between two or more polygons. Overlap refers to the extent of the polygons beyond intersection. An optional report can be created detailing overlapped statistics.
        ...
        variables = params.get("variables")
            ...
            # Generate output name and path
            output_name = f"{in_features.replace(' ', '_')}_Measure_Cannibalization"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Measure Cannibalization
            arcpy.MeasureCannibalization(in_features, area_id_field, area_description_field, out_feature_class, store_id_field, create_report, report_title, report_folder, report_format, variables)
...
Removes overlap between two or more areas to form adjacent boundaries.
        ...
        link_field = params.get("link_field")
            ...
            # Generate output name and path
            output_name = f"{in_features.replace(' ', '_')}_Remove_Overlap"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Remove Overlap
            arcpy.RemoveOverlap(in_features, out_feature_class, method, define_trade_area, ring_id_field, weight_field, store_id, in_stores_layer, link_field)
    ...
    def remove_overlap_(multiple)(self, params):
...
Removes overlap between polygons contained in multiple input layers.
        ...
        join_attributes = params.get("join_attributes")
            ...
            # Generate output name and path
            output_name = f"{in_features.replace(' ', '_')}_Remove_Overlap_(multiple)"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Remove Overlap (multiple)
            arcpy.RemoveOverlap(multiple)(in_features, out_feature_class, method, join_attributes)
