#!/usr/bin/env python3
"""
Improved test script to generate progent.pyt style functions and AI declarations
"""
...
def generate_progent_function(tool_name, parameters, summary=""):
    ...
    param_lines = []
    required_params = []
    param_mapping = {}  # Maps clean names to original names

    for i, param in enumerate(parameters):
        original_name = param["name"]
        clean_name = clean_parameter_name(original_name)
        param_mapping[clean_name] = original_name
    ...
    # Generate output name logic - use first required param or tool name
    if required_params:
        output_base = f"{required_params[0].replace('_', '_')}_{tool_name}"
    else:
        output_base = f"{tool_name}_output"

    # Build arcpy call arguments - use original parameter names for arcpy
    arcpy_args = []
