...
Creates  closed multipatch features from open multipatch features.
        ...
        try:
            ...
            output_name = f"{os.path.basename(in_features).replace(' ', '_')}_Enclose_Multipatch"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            ...
            arcpy.ddd.EncloseMultipatch(in_features, output_path, grid_size)
...
Creates height adjusted breaklines from 3D polygons representing river banks.
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_rivers).replace(' ', '_')}_Enforce_River_Monotonicity"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Enforce River Monotonicity
            arcpy.ddd.EnforceRiverMonotonicity(in_rivers, in_flow_direction, output_path, max_sample_distance, simplification_tolerance)
...
Creates 3D point features along 3D lines using three-dimensional distances.
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_features).replace(' ', '_')}_Generate_Points_Along_3D_Lines"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Generate Points Along 3D Lines
            arcpy.ddd.GeneratePointsAlong3DLines(in_features, output_path, point_placement, distance, percentage, include_end_points, add_chainage_fields, distance_field)
...
Evaluates multipatch features to determine whether each feature completely encloses a volume of space.
        ...
        try:
            ...
            # This tool does not create an output feature class, but modifies the input
            arcpy.ddd.IsClosed3D(in_feature_class)

            return {"success": True, "message": f"IsClosed3D analysis complete on {in_feature_class}."}
...
Generalizes 3D line features to reduce the overall number of vertices while approximating the original shape in horizontal and vertical directions within a specified tolerance.
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_features).replace(' ', '_')}_Simplify_3D_Line"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Simplify 3D Line
            arcpy.ddd.Simplify3DLine(in_features, output_path, tolerance)
...
Imports 3D features from one or more ASCII files stored in XYZ, XYZI, or GENERATE formats into a new feature class.
        ...
        params: {"input_path": <Folder; File>, "in_file_type": <String>, "out_feature_class": <Feature Class>, ...}
        Returns: {"success": True, "output_layer": <output_name>, "output_path": <output_path>} or error
        """
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(input_path).replace(' ', '_')}_ASCII_3D_To_Feature_Class"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute ASCII 3D To Feature Class
            arcpy.ddd.ASCII3DToFeatureClass(input_path, in_file_type, output_path, out_geometry_type, z_factor, input_coordinate_system, average_point_spacing, file_suffix, decimal_separator)
...
Exports 3D features to ASCII text files storing GENERATE, XYZ, or profile data.
        ...
        try:
            ...
            arcpy.ddd.FeatureClassZToASCII(in_feature_class, output_location, out_file, file_format, delimiter, decimal_format, digits_after_decimal, decimal_separator)

            output_path = os.path.join(output_location, out_file)
...
Creates 3D features using height values derived from the attribute of the input features.
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_features).replace(' ', '_')}_Feature_To_3D_By_Attribute"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Feature To 3D By Attribute
            arcpy.ddd.FeatureTo3DByAttribute(in_features, output_path, height_field, to_height_field)
...
Generates 3D geometries  from existing 2D and 3D input features using rules authored in ArcGIS CityEngine.
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_features).replace(' ', '_')}_Features_From_CityEngine_Rules"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Features From CityEngine Rules
            arcpy.ddd.FeaturesFromCityEngineRules(in_features, in_rule_package, output_path, in_existing_fields, in_include_reports, in_leaf_shapes)
...
Imports one or more 3D models into a multipatch feature class.
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_files).replace(' ', '_')}_Import_3D_Files"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Import 3D Files
            arcpy.ddd.Import3DFiles(in_files, output_path, root_per_feature, spatial_reference, y_is_up, file_suffix, in_featureclass, symbol_field)
...
Exports feature layers with 3D display properties to 3D lines or multipatch features.
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_feature_layer).replace(' ', '_')}_Layer_3D_To_Feature_Class"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Layer 3D To Feature Class
            arcpy.ddd.Layer3DToFeatureClass(in_feature_layer, output_path, group_field, disable_materials)
...
Creates 3D models of LOD2 buildings using building footprint polygons and a raster elevation surface.
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_height_source).replace(' ', '_')}_Extract_LOD2_Buildings"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Extract LOD2 Buildings
            arcpy.ddd.ExtractLOD2Buildings(in_height_source, in_features, output_path, level_of_detail, smoothness_level, extraction_accuracy)
...
Extracts multipatch features representing objects in an integrated mesh based on a classified point cloud.
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_mesh).replace(' ', '_')}_Extract_Mesh_Features_Using_Point_Cloud"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Extract Mesh Features Using Point Cloud
            arcpy.ddd.ExtractMeshFeaturesUsingPointCloud(in_mesh, in_point_cloud, output_path, class_codes, point_distance_threshold, maximum_triangle_area, cluster_distance, minimum_cluster_area, boundary)
...
Creates a multipatch feature from the portion of an integrated mesh that overlaps a polygon.
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(source_mesh).replace(' ', '_')}_Extract_Multipatch_From_Mesh"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Extract Multipatch From Mesh
            arcpy.ddd.ExtractMultipatchFromMesh(source_mesh, footprint_features, output_path)
...
from classified point cloud data.
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_point_cloud).replace(' ', '_')}_Extract_Power_Lines_From_Point_Cloud"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Extract Power Lines From Point Cloud
            arcpy.ddd.ExtractPowerLinesFromPointCloud(in_point_cloud, class_codes, output_path, point_tolerance, separation_distance, max_sampling_gap, line_tolerance, wind_correction, min_wind_span, max_wind_deviation, end_point_search_radius, min_length, eliminate_wind, min_line_length)
...
Creates building models using rooftop points in a LAS dataset.
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_las_dataset).replace(' ', '_')}_LAS_Building_Multipatch"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute LAS Building Multipatch
            arcpy.ddd.LASBuildingMultipatch(in_las_dataset, in_features, ground, output_path, point_selection, simplification, sampling_resolution, min_height_field, max_height_field)
...
Creates polygon footprints representing the two-dimensional  area of multipatch features.
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_feature_class).replace(' ', '_')}_Multipatch_Footprint"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Multipatch Footprint
            arcpy.ddd.MultipatchFootprint(in_feature_class, output_path, group_field)
...
Regularizes building footprints that have common boundaries.
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_features).replace(' ', '_')}_Regularize_Adjacent_Building_Footprint"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Regularize Adjacent Building Footprint
            arcpy.ddd.RegularizeAdjacentBuildingFootprint(in_features, group, output_path, method, tolerance, precision, angular_limit)
...
Normalizes the footprint of building polygons by eliminating undesirable artifacts in their geometry.
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_features).replace(' ', '_')}_Regularize_Building_Footprint"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Regularize Building Footprint
            arcpy.ddd.RegularizeBuildingFootprint(in_features, output_path, method, tolerance, densification, precision, diagonal_penalty, min_radius, max_radius, alignment_feature, alignment_tolerance, tolerance_type)
...
Creates surface-conforming  multipatch features by draping polygon features over a surface.
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_surface).replace(' ', '_')}_Interpolate_Polygon_To_Multipatch"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Interpolate Polygon To Multipatch
            arcpy.ddd.InterpolatePolygonToMultipatch(in_surface, in_feature_class, output_path, max_strip_size, z_factor, area_field, surface_area_field, pyramid_level_resolution)
...
Creates 3D features by interpolating z-values from a surface. Learn more about how Interpolate Shape works
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_surface).replace(' ', '_')}_Interpolate_Shape"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Interpolate Shape
            arcpy.ddd.InterpolateShape(in_surface, in_feature_class, output_path, sample_distance, z_factor, method, vertices_only, pyramid_level_resolution, preserve_features)
...
Updates the z-coordinates of 3D feature vertices using a surface.
        ...
        try:
            ...
            # This tool modifies the input feature class
            arcpy.ddd.UpdateFeatureZ(in_features, in_surface, method, status_field)

            return {"success": True, "message": f"Update Feature Z complete on {in_features}."}
...
Creates features that update the z-values of 3D line or polygon vertices with placeholder values that represent missing z-value information.
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_features).replace(' ', '_')}_Calculate_Missing_Z_Values"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Calculate Missing Z Values
            arcpy.ddd.CalculateMissingZValues(in_features, output_path, placeholder)
...
Identifies anomalous elevation measurements from terrain, TIN, or LAS datasets that exceed a defined range of elevation values or have slope characteristics that are inconsistent with the surrounding surface.
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_surface).replace(' ', '_')}_Locate_Outliers"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Locate Outliers
            arcpy.ddd.LocateOutliers(in_surface, output_path, apply_hard_limit, absolute_z_min, absolute_z_max, apply_comparison_filter, z_tolerance, slope_tolerance, exceed_tolerance_ratio, outlier_cap)
...
Creates polygon features that represent aspect measurements derived  from a TIN, terrain, or LAS dataset surface.
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_surface).replace(' ', '_')}_Surface_Aspect"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Surface Aspect
            arcpy.ddd.SurfaceAspect(in_surface, output_path, class_breaks_table, aspect_field, pyramid_level_resolution)
...
Creates contour lines derived from a terrain, TIN, or LAS dataset surface. Learn more about how Surface Contour works
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_surface).replace(' ', '_')}_Surface_Contour"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Surface Contour
            arcpy.ddd.SurfaceContour(in_surface, output_path, interval, base_contour, contour_field, contour_field_precision, index_interval, index_interval_field, z_factor, pyramid_level_resolution)
...
Creates polygon features that represent ranges of slope values for triangulated surfaces.
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_surface).replace(' ', '_')}_Surface_Slope"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Surface Slope
            arcpy.ddd.SurfaceSlope(in_surface, output_path, units, class_breaks_table, slope_field, z_factor, pyramid_level_resolution)
...
Creates line features that represent sight lines from one or more observer points to features in a  target feature class.
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_observer_points).replace(' ', '_')}_Construct_Sight_Lines"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Construct Sight Lines
            arcpy.ddd.ConstructSightLines(in_observer_points, in_target_features, output_path, observer_height_field, target_height_field, join_field, sample_distance, output_the_direction, sampling_method)
...
Determines the visibility of sight lines over obstructions consisting of a surface and an optional multipatch dataset. Learn more about how Line Of Sight works
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_surface).replace(' ', '_')}_Line_Of_Sight"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Line Of Sight
            arcpy.ddd.LineOfSight(in_surface, in_line_feature_class, output_path, out_obstruction_feature_class, use_curvature, use_refraction, refraction_factor, pyramid_level_resolution, in_features, output_graphing_attributes)
...
Identifies which observer points are visible from each raster surface location. The Geodesic Viewshed tool provides enhanced functionality or performance. Learn more about how Observer Points works
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_raster).replace(' ', '_')}_Observer_Points"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Observer Points
            result = arcpy.sa.ObserverPoints(in_raster, in_observer_point_features, output_path, z_factor, curvature_correction, refractivity_coefficient, out_agl_raster)
            result.save(output_path)
...
Generates a multipatch feature class representing a skyline barrier or shadow volume. Learn more about how Skyline Barrier works
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_observer_point_features).replace(' ', '_')}_Skyline_Barrier"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Skyline Barrier
            arcpy.ddd.SkylineBarrier(in_observer_point_features, in_features, output_path, min_radius_value_or_field, max_radius_value_or_field, closed, base_elevation, project_to_plane)
...
Calculates the sky visibility ratio and generates an optional table and a polar graph.
        ...
        try:
            ...
            # This tool has no feature output, it creates a table and a graph
            ...
            return {"success": True, "message": "Skyline Graph created."}
...
Calculates the number of times a fixed position on a surface has its direct sight line to the sun obstructed by multipatch features.
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_features).replace(' ', '_')}_Sun_Shadow_Frequency"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Sun Shadow Frequency
            arcpy.ddd.SunShadowFrequency(in_features, ground, output_path, cell_size, start_time, end_time, time_interval, time_zone, dst, max_shadow_length)
...
Creates closed volumes that model shadows cast by each feature using sunlight for a given date and time.
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_features).replace(' ', '_')}_Sun_Shadow_Volume"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Sun Shadow Volume
            arcpy.ddd.SunShadowVolume(in_features, start_date_and_time, output_path, adjusted_for_dst, time_zone, end_date_and_time, iteration_interval, iteration_unit)
...
Determines the raster surface locations visible to a set of observer features. The Geodesic Viewshed tool provides enhanced functionality or performance. Learn more about how Viewshed works
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_raster).replace(' ', '_')}_Viewshed"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Viewshed
            result = arcpy.sa.Viewshed(in_raster, in_observer_features, output_path, z_factor, curvature_correction, refractivity_coefficient, out_agl_raster)
            result.save(output_path)
...
Determines the raster surface locations visible to a set of observer features, or identifies which observer points are visible from each raster surface location. The Geodesic Viewshed tool provides enhanced functionality or performance.
        ...
        try:
            ...
            # Generate output name and path
            output_name = f"{os.path.basename(in_raster).replace(' ', '_')}_Visibility"
            aprx = arcpy.mp.ArcGISProject("CURRENT")
            output_path = os.path.join(aprx.defaultGeodatabase, output_name)
            output_path = arcpy.CreateUniqueName(output_path)

            # Execute Visibility
            result = arcpy.sa.Visibility(in_raster, in_observer_features, output_path, out_agl_raster, analysis_type, nonvisible_cell_value, z_factor, curvature_correction, refractivity_coefficient, surface_offset, observer_elevation, observer_offset, inner_radius, outer_radius, horizontal_start_angle, horizontal_end_angle, vertical_upper_angle, vertical_lower_angle)
            result.save(output_path)
